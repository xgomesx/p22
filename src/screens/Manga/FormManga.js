...
export default function FormMangas({ route }) {
  const navigation = useNavigation();
  const { acao, manga: mangaAntigo } = route.params;
  ...
  useEffect(() => {
    if (mangaAntigo) {
      setNome(mangaAntigo.nome);
      setCategoria(mangaAntigo.categoria);
      setLancamento(mangaAntigo.lancamento);
      setDiretor(mangaAntigo.diretor);
    }
  }, [mangaAntigo]);
  ...
  return (
    <View style={styles.container}>
      <Text variant="titleLarge" style={styles.title}>
        {mangaAntigo ? 'Editar manga' : 'Adicionar manga'}
      </Text>
